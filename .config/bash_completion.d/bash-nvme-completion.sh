...
#_ext_intel_cmds="id-ctrl internal-log lat-stats \
#	set-bucket-thresholds lat-stats-tracking market-name \
#	smart-log-add temp-stats"
#_ext_intel_cmds+=" version help"
...
plugin_lnvm_opts () {
	...
	case "$1" in
		...
		opts+=" --device-name= -d --erase-only-marked -e 
			...
			;;
		"diag-bbtbl")
		opts+=" --namespace-id= -n --channel-id= -c --lun-id= -l --raw-binary -b"
			;;
		"diag-set-bbtbl")
		opts+=" --namespace-id= -n --channel-id= -c --lun-id= -l --plane-id= -p \
			--block-id= -b --value= -v"
			;;
		"version")
		opts=""
